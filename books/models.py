...
class PurchaseRequest(models.Model):
    """Yêu cầu mua sách - Để xác nhận giao dịch"""
    ...
    message = models.TextField(
        ...
        help_text='Tin nhắn gửi kèm yêu cầu'
    )
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        # Mỗi user chỉ được gửi 1 yêu cầu cho 1 sách
        unique_together = ['book', 'buyer']
    
    def __str__(self):
        return f"{self.buyer.username} -> {self.book.title} ({self.get_status_display()})"
    
    def approve(self):
        """Duyệt yêu cầu - gán buyer và chuyển sách thành đã bán"""
        ...
        # Từ chối các yêu cầu khác của sách này
